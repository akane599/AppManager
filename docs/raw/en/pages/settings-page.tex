...
\begin{itemize}
    ...
    \item \textbf{Shizuku.} Operate App Manager through \href{https://shizuku.rikka.app}{Shizuku} (or Sui). App Manager
    starts its usual privileged server through Shizuku instead of ADB, which offers the same capabilities as
    \textit{ADB over TCP} without having to keep wireless debugging enabled. Shizuku must be installed and running,
    and it will ask you to authorise App Manager the first time. App Manager will fall back to \textit{no-root} mode
    if Shizuku is unavailable or the authorisation is denied.
    \begin{tip}{Info}
        This option requires Android 7 or later. When Sui is used, the server runs as \textit{root} rather than as
        \textit{ADB}.
    \end{tip}
